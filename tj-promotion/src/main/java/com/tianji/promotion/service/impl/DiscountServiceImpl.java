...
public class DiscountServiceImpl implements IDiscountService {
    ...
    @Resource(name = "discountSolutionExecutor")
    private Executor discountSolutionExecutor; // 注入计算优惠方案的自定义线程池
    ...
    public List<CouponDiscountDTO> findDiscountSolution(List<OrderCourseDTO> orderCourses) {
        ...
        // 4.计算方案的优惠明细
        List<CouponDiscountDTO> list = Collections.synchronizedList(new ArrayList<>(solutions.size())); // 对ArrayList进行封装
        // 4.1.定义闭锁
        CountDownLatch latch = new CountDownLatch(solutions.size());
        for (List<Coupon> solution : solutions) {
            // 4.2.异步计算
            CompletableFuture.supplyAsync(() -> calculateSolutionDiscount(availableCouponMap, orderCourses, solution), discountSolutionExecutor)
                    .thenAccept(dto -> {
                        // 4.3.提交任务结果
                        list.add(dto);
                        latch.countDown();
    ...
    /**
     * Calculate discount detailed info according to the user's order
    ...
    /**
     * Find the best coupons solutions
     * @param list
     * @return
     */
    private List<CouponDiscountDTO> findBestSolution(List<CouponDiscountDTO> list) {
        ...
        for (CouponDiscountDTO solution : list) {
            ...
            if (size > 1 && best != null && best.getIds().size() <= size) { // size > 1防止单券方案相互淘汰，保留所有单券方案
                // 当前方案用券更多，放弃
                continue;
            }
            // 2.4.更新最优解
    ...
    /**
     * Calculate the discount amount for a coupon-used solution
     * @param couponMap
     * @param courses
     * @param solution
     * @return
    ...
    /**
     * Update discount amount for each course
     * @param detailMap
     * @param courses
     * @param totalAmount
    ...
    /**
     * Filter available coupons for given courses according to use scope of each coupon
     * Currently, the use scope is just the category a course belongs to.
     * @param coupons
     * @param courses
     * @return
     */
