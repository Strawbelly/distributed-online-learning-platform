...
public class InteractionReplyServiceImpl extends ServiceImpl<InteractionReplyMapper, InteractionReply> implements IInteractionReplyService {
    ...
    public void saveReply(ReplyDTO replyDTO) {
        ...
        questionService.lambdaUpdate()
                ...
                .set(replyDTO.getIsStudent(), InteractionQuestion::getStatus, QuestionStatus.UN_CHECK.getValue()) // 隐含业务，每次新增一条问题下的回复或评论都要将问题状态设置为未查看
    ...
    public PageDTO<ReplyVO> queryReplyPage(ReplyPageQuery query, boolean forAdmin) {
        ...
        // 标记当前是查询问题下的回答
        boolean isQueryAnswer = questionId != null; // true说明查询的是问题下的回答
        // 2.分页查询reply
        Page<InteractionReply> page = lambdaQuery()
                .eq(isQueryAnswer, InteractionReply::getQuestionId, questionId)
                .eq(InteractionReply::getAnswerId, isQueryAnswer ? 0L : answerId) // answer_id默认值是0，如果查询的是问题下的回复，那么就要传入0L
        ...
        Set<Long> answerIds = new HashSet<>(); // 收集回复/评论id
        ...
        // 3.4 查询用户点赞状态
        Set<Long> bizLiked = remarkClient.isBizLiked(answerIds); // 查询当前用户点赞了哪些回复/评论，返回的是id
        // 4.处理VO
        ...
        for (InteractionReply r : records) {
            ...
            if(r.getTargetReplyId() != null){
                ...
                if (targetUser != null) { // 目标如果是匿名的，就查不到用户信息
                    v.setTargetUserName(targetUser.getName());
                }
            }
            // 4.4 点赞状态
            v.setLiked(bizLiked.contains(r.getId())); // 如果bizLiked集合包含当前id，说明用户点过赞
        }
